...
public class MongoKit {
	...
	public static String saveImage(File file, String collectionName) throws Exception {
	...
	public static void findImage(String id, ServletOutputStream _outputStream, String collectionName) throws Exception {
		GridFS gridFS = new GridFS(defaultDb, collectionName);
	...
	public static String findImageToBase64String(String id, String collectionName) {
		try {
			GridFS gridFS = new GridFS(defaultDb, collectionName);
			ObjectId _objectId = new ObjectId(id);
			GridFSDBFile fsFile = gridFS.findOne(_objectId);
			byte[] imageData = new byte[(int) fsFile.getLength()];
			fsFile.getInputStream().read(imageData);
	...
	public static void deleteImage(String id, String collectionName) throws Exception {
		GridFS gridFS = new GridFS(defaultDb, collectionName);
		ObjectId _objectId = new ObjectId(id);
	...
	public static long collectionTotal(String collectionName) {
		DBCollection logs = MongoKit.getCollection(collectionName);
		long num = logs.count();
		return num;
	}
